...
import { CreditCard, MapPin, Calendar, Check, AlertTriangle, X, Copy, MessageCircle } from "lucide-react";
...
export default function PlayerCard({ player }: { player: Player }) {
  ...
  const handlePaymentMade = async () => {
    ...
    if (!error) {
      alert('Thank you! Your status has been updated to Pending. Our finance team will verify it shortly.');
      setShowModal(false);
  ...
  return (
    <>
      <div className="group bg-white border border-gray-200 rounded-xl p-5 hover:border-amber-400 hover:bg-gray-100 hover:shadow-[0_0_20px_rgba(251,191,36,0.25)] transition-all duration-300 flex flex-col relative">
        ...
        <div className="w-full aspect-square bg-gray-50 rounded-lg mb-4 flex items-center justify-center overflow-hidden border border-gray-100">
          {player.image_url ? (
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start mb-1">
            <h4 className="text-lg font-bold text-gray-900 leading-tight">{player.name}</h4>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden border border-gray-100 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 pb-20"> 
              ...
              <div className="mb-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-4">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                    ...
                    <button 
                      onClick={handleCopy}
                      className="p-2.5 bg-gray-200 rounded-lg hover:bg-amber-400 hover:text-white transition-colors flex-shrink-0 ml-2"
                      title="Copy IBAN"
                    >
                      <Copy size={18} />
                    </button>
              ...
              <button 
                ...
                className="w-full py-3.5 bg-emerald-500 text-white font-bold rounded-lg hover:bg-emerald-600 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20"
              >
            ...
            <a 
              href={`https://wa.me/${supportWhatsApp}?text=Hello 11WINS Support, I need help with a payment for ${player.name}.`}
              target="_blank"
              rel="noopener noreferrer"
              className="absolute bottom-5 left-6 flex items-center gap-2 text-xs font-medium text-gray-400 hover:text-amber-500 transition-colors"
            >
